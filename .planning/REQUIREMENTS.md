...
- [ ] **SIM-03**: Кибербакап: прямое восстановление с ленты на объекты (10 потоков, скорость = суммарная скорость всех драйвов / потоки, лимит — скоростная сеть)
- [ ] **SIM-04**: Конкурент: двухфазное восстановление — сначала лента → СХД (суммарная скорость всех драйвов, лимит — скоростная сеть), затем СХД → объекты (10 потоков, лимит — ЛВС)
- [ ] **SIM-05**: Восстановление идёт в порядке приоритета: БД → Серверы → Файловые хранилища → Рабочие станции
- [ ] **SIM-06**: Данные распределяются равномерно между всеми ленточными библиотеками
- [ ] **SIM-07**: В каждом триале стохастически моделируются: снижение доступности IT-инженеров, задержки запуска систем, вариации скорости сети (±20%)
- [ ] **SIM-08**: Поправка на неопределённость применяется как wave-level мультипликатор (одно значение на волну на триал)
- [ ] **SIM-09**: Время запуска после восстановления: БД/Сервер/Файловый сервер — 15 мин (то же оборудование) или 1–3 часа (другое); Рабочая станция — 30 мин–2 часа
